...
from eval.demo.bugs import BUG_TYPES, BUGS, TASKS
...
def test_all_four_bug_types_covered():
    ...
    assert len(TASKS) == 3
    assert len(BUGS) == 16
    assert {b.task_id for b in BUGS} == set(TASKS)
...
def _route_status(root, bug):
    # 从指定 root 全新导入对应 api 模块，拿到这份代码的行为（前后清缓存，避免串味）
    module_name = ".".join(Path(bug.file).with_suffix("").parts)
    top_module = module_name.split(".", 1)[0]

    def _drop_top_module():
        for name in [n for n in sys.modules if n == top_module or n.startswith(f"{top_module}.")]:
    ...
    try:
        ...
        return TestClient(api.app).get(bug.probe).status_code
    finally:
...
def test_route_bug_actually_changes_behavior(bug, tmp_path):
    ...
    assert _route_status(clean_root, bug) == bug.clean_value
    assert _route_status(var_root, bug) == bug.buggy_value
