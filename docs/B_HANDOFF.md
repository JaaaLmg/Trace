...
- 主要 FastAPI 返回已补显式 `response_model`，C 线以 `app/schemas/api_*.py` 为接口契约。
- Alembic 迁移已覆盖 V1 baseline 和 `test_runs.test_plan_id` 收紧；自动化测试会对比 Alembic 与 ORM 建表结果。
- 当前已验证真实异步 smoke：`queued -> completed`，并成功写入：
  ...
  - `run_artifacts`
  - `test_reports`
- 自动化测试已覆盖 Celery `task_always_eager=True` 路径，验证 API 入队后任务函数真实执行 run。
...
B 没有重写 Agent，只是在 [backend/app/services/test_runs.py](../backend/app/services/test_runs.py) 里补齐了：
...
- `save_report` 会先按 report id 查，再按 `run_id` 兜底更新，避免同一 run 重写报告撞唯一约束
...
- [backend/app/models/strategy.py](../backend/app/models/strategy.py)
- [backend/app/models/test_plan.py](../backend/app/models/test_plan.py)
- [backend/app/models/test_run.py](../backend/app/models/test_run.py)
- [backend/app/models/artifacts.py](../backend/app/models/artifacts.py)
- [backend/app/models/trace.py](../backend/app/models/trace.py)
...
- `test_runs.test_plan_id` 非空，删除 plan 时采用 `RESTRICT`，避免孤儿 run
...
- [backend/app/api/routes/test_plans.py](../backend/app/api/routes/test_plans.py)
- [backend/app/api/routes/test_runs.py](../backend/app/api/routes/test_runs.py)
- [backend/app/api/routes/strategy_versions.py](../backend/app/api/routes/strategy_versions.py)
...
这一步在 [backend/app/services/test_runs.py](../backend/app/services/test_runs.py) 的 `create_run(...)` 中完成，主要做：
...
- [backend/scripts/run_api.py](../backend/scripts/run_api.py)
...
- [backend/scripts/run_worker.py](../backend/scripts/run_worker.py)
...
- [backend/scripts/spawn_dev_services.py](../backend/scripts/spawn_dev_services.py)

### 做一次真实异步 smoke

- [backend/scripts/smoke_async_run.py](../backend/scripts/smoke_async_run.py)
...
- `cancel` 是软取消：worker 消费前会跳过已取消 run，但不会终止已经启动的子进程；recorder 会保护 `cancelled` 终态，避免后续完成回写覆盖它。
- 主要 API 已有显式 response schema；后续新增接口需要继续保持这个约束。
- `execute_run_sync(...)` 当前默认走 MockLLM。
- `runtime_profile_id` 还只是预留，没有真正的 runtime profile 模型。
- `project_snapshots` 当前是路径快照，不是代码内容拷贝。
- 数据库结构现在以 Alembic 为准，启动服务前应先执行 `python -m alembic upgrade head`。

## 一次真实踩坑记录：API 集成测试里的 PostgreSQL 死锁

这部分单独记录，是为了避免后续团队成员看到 `DeadlockDetected` 时误判成业务事务设计错误。

### 现象

在 2026-06-14 的本地回归中，`test_api_async_run.py::test_async_run_is_enqueued` 偶发失败，报错类似：

- `psycopg.errors.DeadlockDetected`
- 锁竞争表集中在 `test_plans` 和 `test_runs`

从表面上看，很像是 `create_run()` 的插入事务本身有死锁。

### 实际根因

根因不在业务逻辑，而在测试隔离：

1. API 测试里 `TestClient` 没有统一使用上下文管理。
2. 上一条测试结束后，请求线程和数据库连接释放不够及时。
3. 下一条测试一开始，`clean_db` 就执行 `TRUNCATE ... CASCADE`。
4. Windows + PostgreSQL 本地环境下，这种“上一条测试刚释放、下一条测试立刻清库”的时序容易造成短暂锁竞争。

也就是说，这不是：

- `create_run()` 自己设计出了跨表死锁
...
这样每条测试结束时，FastAPI/Starlette 的生命周期、请求线程和连接都能更及时回收。

2. 在 [backend/tests/conftest.py](../backend/tests/conftest.py) 的 `reset_trace_db()` 中：

- 对 `TRUNCATE` 设置短 `lock_timeout`
- 对短暂 `OperationalError` 做轻量重试

这里的重试不是为了吞掉真实业务错误，而是为了把“测试环境锁释放的瞬时抖动”和“后端逻辑错误”分开。

### 后续经验

后续写 B 线数据库/API 集成测试时，建议默认遵守：

1. `TestClient` 一律使用 `with TestClient(app) as client:`
2. 清库 fixture 不要假设上一条测试结束后锁一定已经完全释放
3. 如果出现 PostgreSQL 死锁，先排查测试清理和客户端生命周期，再怀疑业务事务顺序
...
- 保持干净数据库上的 `alembic upgrade head` 验证，并继续维护 schema migration 对照测试
...
- [backend/app/services/test_runs.py](../backend/app/services/test_runs.py)：B 线运行编排核心
- [backend/app/recorders/sqlalchemy_recorder.py](../backend/app/recorders/sqlalchemy_recorder.py)：A/B 适配核心
- [backend/app/workers/tasks.py](../backend/app/workers/tasks.py)：Celery 消费入口
- [backend/app/api/routes/test_runs.py](../backend/app/api/routes/test_runs.py)：run 控制与查询 API
- [backend/app/models/](../backend/app/models)：14 张 V1 表定义
- [backend/scripts/smoke_async_run.py](../backend/scripts/smoke_async_run.py)：真实异步链路 smoke 验证脚本
